...
private:
  ...
  void writer()
  {
    while (!stop_)
    {
      task_node_t *select_task;
      {
        ...
        {
          task_cv_.wait(lock, [this]
                        { return !initial_tasks_.empty() || stop_; });
        }
  ...
  void reader()
  {
    while (!stop_)
    {
      ...
      if (result_to_process <= num_rpi_)
      { // it's rpi_id, we will release resource - HERE IS THE ISSUUEEEEEE
        resource_scheduler.produce_resource(result_to_process);
        continue;
      }
      else
      { // ita task id, we will process real result
